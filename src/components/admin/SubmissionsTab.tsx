...
export default function SubmissionsTab() {
  ...
  const columns: ColumnDef<Submission>[] = [
    ...
    { 
      accessorKey: 'grader.full_name', 
      header: 'Graded By',
      cell: ({ row }) => row.original.grader?.full_name || '-'
    },
  ];
  ...
  return (
    <Card>
      ...
      <CardContent className="space-y-4">
        <div>
          <Label htmlFor="course_filter">Filter by Course</Label>
          <Select 
            value={selectedCourse ? String(selectedCourse) : 'all'} 
            onValueChange={(v) => setSelectedCourse(v === 'all' ? undefined : Number(v))}
          >
            <SelectTrigger>
